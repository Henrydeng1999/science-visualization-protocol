...
function readParams(){return {coherence: parseFloat(select('#coherence').value()),spread: parseFloat(select('#spread').value()),bandwidth: parseFloat(select('#bandwidth').value())};}
function setupControls(){select('#coherence').input(updateLabels);select('#spread').input(updateLabels);select('#bandwidth').input(updateLabels);}
function updateLabels(){select('#coherence-value').html(select('#coherence').value() + '');select('#spread-value').html(select('#spread').value() + ' °');select('#bandwidth-value').html(select('#bandwidth').value() + ' nm');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('激光与普通光', '发光强度 cd'); drawScene(p); }
