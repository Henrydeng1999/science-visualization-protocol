...
function readParams(){return {m1: parseFloat(select('#m1').value()),m2: parseFloat(select('#m2').value()),distance: parseFloat(select('#distance').value())};}
function setupControls(){select('#m1').input(updateLabels);select('#m2').input(updateLabels);select('#distance').input(updateLabels);}
function updateLabels(){select('#m1-value').html(select('#m1').value() + '');select('#m2-value').html(select('#m2').value() + '');select('#distance-value').html(select('#distance').value() + ' px');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('万有引力', '质量 kg · 长度 m'); drawScene(p); }
function drawFrame(title, qty){ noStroke(); fill(230); textSize(17); text(title,24,30); fill(150,165,190); textSize(12); text('关联基本量：'+qty,24,50); }
...
function drawGravity(p){
  const center=createVector(330,255);
  const r=p.distance;
  const force=p.m1*p.m2/(r*r);
  const omega=0.012*sqrt(p.m1/20)*pow(180/r,1.5);
  const theta=frameCount*omega;
  const planet=createVector(center.x+r*cos(theta),center.y+r*sin(theta));
  const tangent=createVector(-sin(theta),cos(theta));
  const inward=p5.Vector.sub(center,planet).normalize();

  drawPolarFrame(center,r,theta);
  drawStar(center,p.m1);
  drawPlanet(planet,p.m2);
  drawForceArrow(planet,inward,force);
  drawVelocityVector(planet,tangent,r,p.m1);
  drawGravityReadout(p,force,omega);
  caption(['万有引力提供向心作用：行星速度方向沿轨道切线，引力方向始终指向中心天体。','F ∝ Mm/r²；中心质量越大，稳定公转所需速度越大；距离越远，公转越慢。']);
}

function drawPolarFrame(center,r,theta){
  noFill();
  stroke(40,54,78);
  strokeWeight(1);
  for(let rr=60;rr<=300;rr+=40){
    circle(center.x,center.y,rr*2);
    noStroke();
    fill(95,115,145);
    textSize(10);
    text(rr+'',center.x+rr+5,center.y-4);
    noFill();
    stroke(40,54,78);
  }
  ...
  for(let deg=0;deg<360;deg+=30){
    const a=radians(deg);
    line(center.x,center.y,center.x+305*cos(a),center.y+305*sin(a));
  }
  drawingContext.setLineDash([]);
  stroke(120,220,255,145);
  strokeWeight(2);
  circle(center.x,center.y,r*2);
  drawingContext.setLineDash([6,7]);
  stroke(255,210,100,95);
  ...
  textSize(12);
  fill(255,210,100);
  text('r = '+r.toFixed(0)+' px',center.x+r*cos(theta)/2+8,center.y+r*sin(theta)/2-8);
  drawThetaArc(center,theta);
}

function drawThetaArc(center,theta){
  const arcR=42;
  noFill();
  stroke(255,210,100,135);
  strokeWeight(2);
  const end=theta%(TWO_PI);
  arc(center.x,center.y,arcR*2,arcR*2,0,end);
  noStroke();
  fill(255,210,100);
  textSize(12);
  text('θ',center.x+arcR+8,center.y+16);
}

function drawStar(center,mass){
  const radius=28+mass*0.35;
  noStroke();
  fill(255,190,80,60);
  circle(center.x,center.y,radius*2.6);
  fill(255,210,100);
  circle(center.x,center.y,radius*2);
  fill(255,245,180);
  circle(center.x-8,center.y-8,radius*0.65);
  fill(40,32,18);
  textAlign(CENTER,CENTER);
  textSize(12);
  text('M₁',center.x,center.y+2);
  textAlign(LEFT,BASELINE);
}

function drawPlanet(pos,mass){
  const radius=13+mass*0.12;
  noStroke();
  fill(120,200,255,65);
  circle(pos.x,pos.y,radius*2.4);
  fill(120,200,255);
  circle(pos.x,pos.y,radius*2);
  fill(210,240,255);
  circle(pos.x-4,pos.y-4,radius*0.55);
  fill(12,24,36);
  textAlign(CENTER,CENTER);
  textSize(11);
  text('M₂',pos.x,pos.y+1);
  textAlign(LEFT,BASELINE);
}

function drawForceArrow(pos,dir,force){
  const len=constrain(30+force*2200,32,95);
  drawVector(pos,dir.copy().mult(len),color(120,240,170),'F 引力');
}

function drawVelocityVector(pos,tangent,r,centralMass){
  const len=constrain(28+sqrt(centralMass/r)*120,30,78);
  drawVector(pos,tangent.copy().mult(len),color(255,210,100),'v');
}

function drawVector(origin,v,c,label){
  const start=p5.Vector.add(origin,p5.Vector.mult(v.copy().normalize(),16));
  const end=p5.Vector.add(start,v);
  stroke(c);
  strokeWeight(3);
  ...
  fill(210);
  textSize(11);
  text(label,end.x+6,end.y-6);
}

function drawGravityReadout(p,force,omega){
  noStroke();
  fill(18,25,40,235);
  rect(462,94,178,116,8);
  fill(220);
  textSize(13);
  text('中心质量 M = '+p.m1.toFixed(0),478,120);
  text('行星质量 m = '+p.m2.toFixed(0),478,142);
  text('距离 r = '+p.distance.toFixed(0)+' px',478,164);
  fill(120,240,170);
  text('相对引力 F ≈ '+(force*1000).toFixed(2),478,186);
  fill(255,210,100);
  text('角速度 ω ≈ '+omega.toFixed(3),478,204);
}
