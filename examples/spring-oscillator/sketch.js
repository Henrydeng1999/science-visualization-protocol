...
function readParams(){return {mass: parseFloat(select('#mass').value()),stiffness: parseFloat(select('#stiffness').value()),amplitude: parseFloat(select('#amplitude').value())};}
function setupControls(){select('#mass').input(updateLabels);select('#stiffness').input(updateLabels);select('#amplitude').input(updateLabels);}
function updateLabels(){select('#mass-value').html(select('#mass').value() + ' kg');select('#stiffness-value').html(select('#stiffness').value() + ' N/m');select('#amplitude-value').html(select('#amplitude').value() + ' px');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('弹簧振子', '时间 s · 质量 kg'); drawScene(p); }
...
function drawSpring(p){
  const wallX=110, mid=350, y=245;
  const bobR=28;
  const omega=sqrt(p.stiffness/p.mass);
  const T=TWO_PI/sqrt(p.stiffness/p.mass);
  const displacement=p.amplitude*cos(frameCount*0.05*omega);
  const bobX=mid+displacement;
  const springEnd=bobX-bobR;

  stroke(80,96,125);
  strokeWeight(3);
  line(80,y+bobR,620,y+bobR);
  noStroke();
  fill(42,52,70);
  rect(wallX-18,y-82,18,164,3);
  for(let yy=y-76; yy<y+80; yy+=18){
    stroke(65,78,100);
    line(wallX-18,yy,wallX-2,yy+12);
  }

  drawingContext.setLineDash([8,8]);
  stroke(255,210,100);
  strokeWeight(2);
  line(mid,y-96,mid,y+94);
  drawingContext.setLineDash([]);
  noStroke();
  fill(255,210,100);
  textSize(12);
  text('平衡位置 x = 0',mid+12,y-72);

  stroke(120,140,170);
  strokeWeight(1);
  line(mid-p.amplitude,y-56,mid-p.amplitude,y+72);
  line(mid+p.amplitude,y-56,mid+p.amplitude,y+72);
  noStroke();
  fill(135,150,175);
  textSize(12);
  text('-A',mid-p.amplitude-8,y+92);
  text('+A',mid+p.amplitude-8,y+92);

  stroke(100,190,255);
  strokeWeight(3);
  noFill();
  line(wallX,y,wallX+22,y);
  beginShape();
  const coils=12;
  const coilStart=wallX+22;
  const coilEnd=springEnd-16;
  const samples=coils*18;
  for(let i=0;i<=samples;i++){
    const t=i/samples;
    const sx=lerp(coilStart,coilEnd,t);
    const sy=y+sin(t*coils*TWO_PI)*20;
    vertex(sx,sy);
  }
  endShape();
  line(coilEnd,y,springEnd,y);

  noStroke();
  fill(255,200,90);
  circle(bobX,y,bobR*2);
  fill(255,232,150);
  circle(bobX-9,y-10,12);
  fill(70,45,12);
  textSize(13);
  ...
  drawDoubleArrow(mid,y+118,bobX,y+118,color(255,210,100));
  noStroke();
  fill(255,210,100);
  textSize(12);
  text('x = '+displacement.toFixed(1)+' px',min(mid,bobX)+abs(displacement)/2-34,y+137);

  const force=-p.stiffness*displacement/60;
  drawForceArrow(bobX,y-54,force,color(120,240,170));
  fill(120,240,170);
  textSize(12);
  text('回复力 F = -kx',bobX-44,y-76);

  caption(['周期 T ≈ 2π√(m/k) = '+T.toFixed(2)+' s','虚线是平衡位置；位移越大，回复力越强且方向指向平衡位置。']);
}
function drawDoubleArrow(x1,y1,x2,y2,c){
  stroke(c); strokeWeight(2); line(x1,y1,x2,y2);
  noStroke(); fill(c);
  const dir=x2>=x1?1:-1;
  triangle(x2,y2,x2-dir*8,y2-5,x2-dir*8,y2+5);
  triangle(x1,y1,x1+dir*8,y1-5,x1+dir*8,y1+5);
}
function drawForceArrow(x,y,len,c){
  if(abs(len)<3) return;
  const shown=constrain(len,-72,72);
  const x2=x+shown;
  stroke(c); strokeWeight(3); line(x,y,x2,y);
  noStroke(); fill(c);
  const dir=shown>=0?1:-1;
  triangle(x2,y,x2-dir*9,y-5,x2-dir*9,y+5);
}
