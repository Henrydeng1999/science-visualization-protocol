...
function readParams(){return {a: parseFloat(select('#a').value()),b: parseFloat(select('#b').value())};}
function setupControls(){select('#a').input(updateLabels);select('#b').input(updateLabels);}
function updateLabels(){select('#a-value').html(select('#a').value() + ' 份');select('#b-value').html(select('#b').value() + ' 份');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('化学反应配比', '物质的量 mol'); drawScene(p); }
