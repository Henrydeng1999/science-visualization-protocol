...
function readParams(){return {mass: parseFloat(select('#mass').value()),molar: parseFloat(select('#molar').value())};}
function setupControls(){select('#mass').input(updateLabels);select('#molar').input(updateLabels);}
function updateLabels(){select('#mass-value').html(select('#mass').value() + ' g');select('#molar-value').html(select('#molar').value() + ' g/mol');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('摩尔质量换算', '物质的量 mol'); drawScene(p); }
