...
function readParams(){return {current: parseFloat(select('#current').value()),direction: parseFloat(select('#direction').value())};}
function setupControls(){select('#current').input(updateLabels);select('#direction').input(updateLabels);}
function updateLabels(){select('#current-value').html(select('#current').value() + ' A');select('#direction-value').html(select('#direction').value() + '');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('通电导线磁场', '电流 A'); drawScene(p); }
