...
function readParams(){return {frequency: parseFloat(select('#frequency').value()),intensity: parseFloat(select('#intensity').value()),work: parseFloat(select('#work').value())};}
function setupControls(){select('#frequency').input(updateLabels);select('#intensity').input(updateLabels);select('#work').input(updateLabels);}
function updateLabels(){select('#frequency-value').html(select('#frequency').value() + ' 相对值');select('#intensity-value').html(select('#intensity').value() + '');select('#work-value').html(select('#work').value() + ' 相对值');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('光电效应', '发光强度 cd · 量子入口'); drawScene(p); }
