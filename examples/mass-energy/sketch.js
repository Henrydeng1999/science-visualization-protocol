...
const reactionData={
  fission:{name:'核裂变',massLoss:0.21,energy:200,unit:'MeV/次'},
  fusion:{name:'核聚变',massLoss:0.019,energy:17.6,unit:'MeV/次'}
};

function readParams(){return {mode: select('#reaction').value()};}
function setupControls(){select('#reaction').changed(updateLabels);}
function updateLabels(){select('#reaction-value').html(reactionData[select('#reaction').value()].name);}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('质能关系示意', '质量 kg'); drawScene(p); }
...
function drawMassEnergy(p){
  const data=reactionData[p.mode];
  const t=(frameCount%220)/220;
  drawEnergyPanel(data);
  if(p.mode==='fission') drawFissionAnimation(t,p,data.energy);
  else drawFusionAnimation(t,p,data.energy);
  caption(['质能关系：E = Δm·c²。核反应后产物总质量略小，少掉的质量以能量形式释放。','左侧用同一参照比较单次反应：铀裂变约 200 MeV，D-T 聚变约 17.6 MeV。']);
}

function drawEnergyPanel(data){
  const x=58,y=92,w=232,h=320;
  const maxMass=max(reactionData.fission.massLoss,reactionData.fusion.massLoss);
  const maxEnergy=max(reactionData.fission.energy,reactionData.fusion.energy);
  noStroke();
  fill(18,25,40,235);
  rect(x,y,w,h,8);
  fill(220);
  textSize(15);
  text('同一参照比较',x+18,y+28);
  const massH=map(data.massLoss,0,maxMass,0,120);
  const energyH=map(data.energy,0,maxEnergy,0,205);
  fill(255,190,100);
  rect(x+38,y+238-massH,56,massH,6);
  fill(120,240,170);
  rect(x+138,y+238-energyH,64,energyH,6);
  fill(150,165,190);
  textSize(12);
  text('Δm',x+52,y+258);
  text('E',x+164,y+258);
  stroke(255,210,100,135);
  strokeWeight(2);
  line(x+98,y+178,x+132,y+178);
  noStroke();
  fill(255,210,100);
  triangle(x+132,y+178,x+123,y+173,x+123,y+183);
  noStroke();
  fill(255,210,100);
  textSize(13);
  text('E = Δm · c²',x+60,y+298);
  fill(120,240,170);
  text(data.name+'：'+data.energy.toFixed(1)+' '+data.unit,x+38,y+322);
  fill(255,190,100);
  text('Δm ≈ '+data.massLoss.toFixed(3)+' u',x+38,y+344);
}

function drawFissionAnimation(t,p,energy){
  const cx=470,cy=250;
  drawReactionPanel('核裂变：重核裂成两个较轻原子核');
  const split=smoothstep(constrain((t-0.32)/0.32,0,1));
  const pulse=constrain((t-0.62)/0.18,0,1);
  const neutronX=330+150*min(t/0.3,1);
  drawNucleus(cx-90*split,cy-34*split,28,color(255,170,100),'Ba','钡');
  drawNucleus(cx+96*split,cy+42*split,24,color(120,200,255),'Kr','氪');
  if(split<0.08) drawNucleus(cx,cy,48,color(255,210,100),'U','铀');
  fill(210);
  noStroke();
  circle(neutronX,cy,8);
  textSize(11);
  text('n 中子',neutronX+8,cy-6);
  if(pulse>0) drawEnergyBurst(cx,cy,pulse,energy);
  for(let i=0;i<3;i++){
    const a=-0.7+i*0.7;
    const nT=constrain((t-0.58)/0.34,0,1);
    fill(210);
    circle(cx+30*cos(a)+150*nT*cos(a),cy+30*sin(a)+110*nT*sin(a),7);
  }
}

function drawFusionAnimation(t,p,energy){
  const cx=470,cy=250;
  drawReactionPanel('核聚变：轻核结合成更稳定的原子核');
  const approach=smoothstep(constrain(t/0.48,0,1));
  const merged=t>0.5;
  const pulse=constrain((t-0.50)/0.22,0,1);
  if(!merged){
    drawNucleus(cx-120+105*approach,cy-28,24,color(120,200,255),'D','氘');
    drawNucleus(cx+120-105*approach,cy+28,24,color(255,170,100),'T','氚');
  }else{
    drawNucleus(cx,cy,36,color(255,210,100),'He','氦');
    fill(210);
    noStroke();
    const nT=constrain((t-0.52)/0.34,0,1);
    circle(cx+130*nT,cy-62*nT,8);
    textSize(11);
    text('n 中子',cx+130*nT+8,cy-62*nT-6);
  }
  if(pulse>0) drawEnergyBurst(cx,cy,pulse,energy);
}

function drawReactionPanel(title){
  noStroke();
  fill(18,25,40,170);
  rect(318,92,322,320,8);
  fill(220);
  textSize(14);
  text(title,338,122);
  stroke(45,56,78);
  strokeWeight(1);
  line(338,138,620,138);
}

function drawNucleus(x,y,r,c,symbol,name){
  noStroke();
  fill(red(c),green(c),blue(c),65);
  circle(x,y,r*2.45);
  fill(c);
  circle(x,y,r*2);
  fill(255,245,190);
  circle(x-r*0.28,y-r*0.3,r*0.48);
  fill(20,26,36);
...
function drawEnergyBurst(x,y,t,energy){
  const rays=12;
  const radius=28+t*constrain(energy/25,45,150);
  stroke(255,230,120,200*(1-t));
  strokeWeight(3);
  for(let i=0;i<rays;i++){
    const a=TWO_PI*i/rays+0.15*sin(frameCount*0.08);
    line(x+24*cos(a),y+24*sin(a),x+radius*cos(a),y+radius*sin(a));
  }
  noStroke();
  fill(255,230,120,60*(1-t));
  circle(x,y,radius*1.35);
  fill(255,230,120);
  textSize(13);
  text('能量释放',x+46,y-52);
}
