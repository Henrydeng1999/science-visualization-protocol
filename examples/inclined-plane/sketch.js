...
let activeParams=null, simStartFrame=0, lastCycleIndex=0;

function readParams(){return {rightAngle: parseFloat(select('#right-angle').value()),friction: parseFloat(select('#friction').value())};}
function setupControls(){select('#right-angle').input(updateLabels);select('#friction').input(updateLabels);select('#restart').mousePressed(restartIncline);}
function updateLabels(){select('#right-angle-value').html(select('#right-angle').value() + ' °');select('#friction-value').html(parseFloat(select('#friction').value()).toFixed(2));}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); restartIncline(); }
function restartIncline(){ activeParams=readParams(); simStartFrame=frameCount; lastCycleIndex=0; }
function draw(){ const p=activeParams||readParams(); background(12,16,28); drawFrame('斜面运动', '质量 kg'); drawScene(p); }
...
function drawIncline(p){
  const cycle=300;
  const elapsed=max(0,frameCount-simStartFrame);
  const cycleIndex=floor(elapsed/cycle);
  if(cycleIndex!==lastCycleIndex){
    activeParams=readParams();
    p=activeParams;
    simStartFrame=frameCount;
    lastCycleIndex=0;
  }
  const valley=createVector(330,360);
  const leftTop=createVector(92,168);
  const leftC1=createVector(138,250);
  const leftC2=createVector(206,360);
  const a=radians(p.rightAngle);
  const rightEndX=650;
  const rightEndY=valley.y-(rightEndX-valley.x)*tan(a);
  const startHeight=valley.y-leftTop.y;
  const frictionWorkScale=4;
  const leftPathLen=estimateCubicLength(leftTop,leftC1,leftC2,valley);
  const valleyEnergy=max(0,startHeight-p.friction*frictionWorkScale*leftPathLen);
  const stop=solveRightStop(valley,p.rightAngle,p.friction,valleyEnergy,rightEndX,frictionWorkScale);
  const recoveredHeight=stop.height;
  const stopX=stop.x;
  const stopY=stop.y;
  const phase=(max(0,frameCount-simStartFrame)%cycle)/cycle;
  const path=buildGalileoPath(leftTop,leftC1,leftC2,valley,createVector(stopX,stopY));
  const progress=smootherStep(phase);
  const motion=pointAlongPath(path,progress*path.total);
  const ball=motion.point;

  drawGalileoTrack(leftTop,leftC1,leftC2,valley,rightEndX,rightEndY,stopX,stopY,startHeight,recoveredHeight,p);
  fill(255,210,100);
  noStroke();
  circle(ball.x,ball.y-15,30);
  fill(255,240,170);
  circle(ball.x-6,ball.y-21,7);
  drawMotionArrow(ball,motion.tangent);
  drawGalileoReadout(p,startHeight,valleyEnergy,recoveredHeight,stop.distance);
  caption(['能量计算：谷底剩余能量高度 = 初始高度 - μ·路径长度对应的摩擦损失。','右侧停止点由“上升高度 + 摩擦做功 = 谷底剩余能量”决定；水平面上只由摩擦耗尽动能。']);
}

function solveRightStop(valley,rightAngle,friction,energyHeight,rightEndX,frictionWorkScale){
  const a=radians(rightAngle);
  if(energyHeight<=0) return {x:valley.x,y:valley.y,height:0,distance:0};
  if(a<0.005){
    const distance=friction>0?energyHeight/(friction*frictionWorkScale):rightEndX-valley.x+220;
    return {x:valley.x+distance,y:valley.y,height:0,distance};
  }
  const slopeCost=sin(a)+friction*frictionWorkScale*cos(a);
  const pathDistance=energyHeight/max(0.001,slopeCost);
  const height=pathDistance*sin(a);
  return {x:valley.x+pathDistance*cos(a),y:valley.y-height,height,distance:pathDistance};
}

function estimateCubicLength(p0,p1,p2,p3){
  let len=0, prev=p0.copy();
  for(let i=1;i<=80;i++){
    const q=cubicPoint(p0,p1,p2,p3,i/80);
    len+=p5.Vector.dist(prev,q);
    prev=q;
  }
  return len;
}

function buildGalileoPath(leftTop,leftC1,leftC2,valley,stop){
  const points=[];
  for(let i=0;i<=90;i++) points.push(cubicPoint(leftTop,leftC1,leftC2,valley,i/90));
  const rightSteps=max(2,ceil(p5.Vector.dist(valley,stop)/8));
  for(let i=1;i<=rightSteps;i++) points.push(p5.Vector.lerp(valley,stop,i/rightSteps));
  const lengths=[0];
  let total=0;
...
function pointAlongPath(path,distance){
  if(path.total<=0) return {point:path.points[0].copy(),tangent:createVector(1,0)};
  const d=constrain(distance,0,path.total);
  for(let i=1;i<path.points.length;i++){
    if(path.lengths[i]>=d){
      const span=max(0.001,path.lengths[i]-path.lengths[i-1]);
      const t=(d-path.lengths[i-1])/span;
      const point=p5.Vector.lerp(path.points[i-1],path.points[i],t);
      const tangent=p5.Vector.sub(path.points[i],path.points[i-1]).normalize();
      return {point,tangent};
    }
  }
  const last=path.points.length-1;
  return {point:path.points[last].copy(),tangent:p5.Vector.sub(path.points[last],path.points[last-1]).normalize()};
}

function drawGalileoTrack(leftTop,leftC1,leftC2,valley,rightEndX,rightEndY,stopX,stopY,startHeight,recoveredHeight,p){
  noFill();
  stroke(92,112,145);
  strokeWeight(2);
  line(60,valley.y,660,valley.y);
  stroke(120,220,255);
  strokeWeight(6);
  beginShape();
  for(let i=0;i<=80;i++){
    const q=cubicPoint(leftTop,leftC1,leftC2,valley,i/80);
    vertex(q.x,q.y);
  }
  endShape();
  stroke(120,220,255);
  line(valley.x,valley.y,rightEndX,rightEndY);

  drawingContext.setLineDash([7,7]);
  stroke(255,210,100,130);
  strokeWeight(1.5);
  line(72,leftTop.y,642,leftTop.y);
  stroke(255,130,110,130);
  line(72,valley.y-recoveredHeight,642,valley.y-recoveredHeight);
  drawingContext.setLineDash([]);

  noStroke();
  fill(255,210,100);
  textSize(12);
  text('初始高度',84,leftTop.y-8);
  fill(255,130,110);
  text('摩擦后可回升高度',84,valley.y-recoveredHeight-8);
  fill(150,165,190);
  text('右侧坡度 '+p.rightAngle.toFixed(0)+'°',452,min(392,rightEndY+28));

  stroke(255,130,110,160);
  strokeWeight(2);
  line(stopX,stopY-28,stopX,stopY+28);
  noStroke();
  fill(255,130,110);
  circle(stopX,stopY,8);
}

function drawGalileoReadout(p,startHeight,valleyEnergy,recoveredHeight,stopDistance){
  noStroke();
  fill(18,25,40,235);
  rect(462,96,178,112,8);
  fill(220);
  textSize(13);
  text('右侧坡度 θ = '+p.rightAngle.toFixed(0)+'°',478,122);
  text('摩擦 μ = '+p.friction.toFixed(2),478,144);
  fill(120,220,255);
  text('初始高度：'+startHeight.toFixed(0)+' px',478,166);
  text('谷底能量：'+valleyEnergy.toFixed(0)+' px',478,184);
  fill(255,130,110);
  text('停止距离：'+stopDistance.toFixed(0)+' px',478,202);
}

function drawMotionArrow(ball,tangent){
  const len=32;
  drawArrow(ball.x+18,ball.y-18,ball.x+18+len*tangent.x,ball.y-18+len*tangent.y,color(255,210,100));
}
