...
function readParams(){return {intensity: parseFloat(select('#intensity').value()),distance: parseFloat(select('#distance').value())};}
function setupControls(){select('#intensity').input(updateLabels);select('#distance').input(updateLabels);}
function updateLabels(){select('#intensity-value').html(select('#intensity').value() + ' cd');select('#distance-value').html(select('#distance').value() + ' px');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('距离与亮度平方反比', '发光强度 cd'); drawScene(p); }
