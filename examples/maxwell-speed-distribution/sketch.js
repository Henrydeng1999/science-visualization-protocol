...
function readParams(){return {temperature: parseFloat(select('#temperature').value()),mass: parseFloat(select('#mass').value())};}
function setupControls(){select('#temperature').input(updateLabels);select('#mass').input(updateLabels);}
function updateLabels(){select('#temperature-value').html(select('#temperature').value() + ' K');select('#mass-value').html(select('#mass').value() + ' 相对值');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('麦克斯韦速度分布', '热力学温度 K'); drawScene(p); }
