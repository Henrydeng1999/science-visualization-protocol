...
function readParams(){return {speed: parseFloat(select('#speed').value()),turns: parseFloat(select('#turns').value()),field: parseFloat(select('#field').value())};}
function setupControls(){select('#speed').input(updateLabels);select('#turns').input(updateLabels);select('#field').input(updateLabels);}
function updateLabels(){select('#speed-value').html(select('#speed').value() + '');select('#turns-value').html(select('#turns').value() + ' 匝');select('#field-value').html(select('#field').value() + '');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('电磁感应', '电流 A · 时间 s'); drawScene(p); }
