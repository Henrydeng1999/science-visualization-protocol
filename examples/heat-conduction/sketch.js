...
function readParams(){return {hot: parseFloat(select('#hot').value()),cold: parseFloat(select('#cold').value()),conductivity: parseFloat(select('#conductivity').value())};}
function setupControls(){select('#hot').input(updateLabels);select('#cold').input(updateLabels);select('#conductivity').input(updateLabels);}
function updateLabels(){select('#hot-value').html(select('#hot').value() + ' °C');select('#cold-value').html(select('#cold').value() + ' °C');select('#conductivity-value').html(select('#conductivity').value() + '');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('热传导', '热力学温度 K'); drawScene(p); }
