...
function readParams(){return {moles: parseFloat(select('#moles').value()),scale: parseFloat(select('#scale').value())};}
function setupControls(){select('#moles').input(updateLabels);select('#scale').input(updateLabels);}
function updateLabels(){select('#moles-value').html(select('#moles').value() + ' mol');select('#scale-value').html(select('#scale').value() + '');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('阿伏伽德罗常数', '物质的量 mol'); drawScene(p); }
