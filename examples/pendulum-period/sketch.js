...
function readParams(){return {length: parseFloat(select('#length').value()),gravity: parseFloat(select('#gravity').value()),angle: parseFloat(select('#angle').value())};}
function setupControls(){select('#length').input(updateLabels);select('#gravity').input(updateLabels);select('#angle').input(updateLabels);}
function updateLabels(){select('#length-value').html(select('#length').value() + ' cm');select('#gravity-value').html(select('#gravity').value() + ' m/s²');select('#angle-value').html(select('#angle').value() + ' °');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('单摆周期', '时间 s'); drawScene(p); }
...
function drawPendulum(p){
  const pivot=createVector(350,110);
  const L=p.length*1.7;
  const T=TWO_PI*sqrt(p.length/max(p.gravity,0.1));
  const maxAngle=radians(p.angle);
  const a=maxAngle*cos(frameCount*0.035/T*6);
  const bob=createVector(pivot.x+L*sin(a), pivot.y+L*cos(a));
  const rest=createVector(pivot.x,pivot.y+L);

  stroke(95,115,145);
  strokeWeight(2);
  drawingContext.setLineDash([8,8]);
  line(pivot.x,pivot.y,pivot.x,min(424,rest.y+32));
  drawingContext.setLineDash([]);
  noStroke();
  fill(135,150,175);
  textSize(12);
  text('中轴线 / 平衡位置',pivot.x+12,pivot.y+48);

  noFill();
  stroke(70,90,120);
  strokeWeight(1);
  arc(pivot.x,pivot.y,L*2,L*2,HALF_PI-maxAngle,HALF_PI+maxAngle);
  fill(70,90,120);
  noStroke();
  circle(rest.x,rest.y,7);

  const pendulumAngle=HALF_PI-a;
  const angleStart=min(HALF_PI,pendulumAngle);
  const angleEnd=max(HALF_PI,pendulumAngle);
  const guideR=58;
  noFill();
  stroke(255,210,100);
  strokeWeight(2);
  arc(pivot.x,pivot.y,guideR*2,guideR*2,angleStart,angleEnd,OPEN);
  stroke(255,210,100,130);
  strokeWeight(1);
  line(pivot.x,pivot.y,pivot.x,pivot.y+guideR);
  line(pivot.x,pivot.y,pivot.x+guideR*sin(a),pivot.y+guideR*cos(a));
  fill(255,210,100);
  noStroke();
  textSize(13);
  text('θ = '+degrees(a).toFixed(1)+'°',pivot.x+44,pivot.y+66);
  ...
  noStroke();
  fill(230);
  circle(pivot.x,pivot.y,12);
  fill(255,210,100);
  circle(bob.x,bob.y,34);
  fill(60,45,18);
  circle(bob.x-5,bob.y-5,7);

  caption(['周期 T ≈ 2π√(L/g) = '+T.toFixed(2)+' s','虚线是中轴线：小球经过这里时速度最大、回复力矩方向改变。']);
}
