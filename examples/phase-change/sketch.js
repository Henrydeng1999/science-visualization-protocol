...
function readParams(){return {power: parseFloat(select('#power').value()),mass: parseFloat(select('#mass').value()),time: parseFloat(select('#time').value())};}
function setupControls(){select('#power').input(updateLabels);select('#mass').input(updateLabels);select('#time').input(updateLabels);}
function updateLabels(){select('#power-value').html(select('#power').value() + '');select('#mass-value').html(select('#mass').value() + '');select('#time-value').html(select('#time').value() + '');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('相变过程', '热力学温度 K'); drawScene(p); }
