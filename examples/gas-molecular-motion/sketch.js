...
function readParams(){return {temperature: parseFloat(select('#temperature').value()),particles: parseFloat(select('#particles').value()),volume: parseFloat(select('#volume').value())};}
function setupControls(){select('#temperature').input(updateLabels);select('#particles').input(updateLabels);select('#volume').input(updateLabels);}
function updateLabels(){select('#temperature-value').html(select('#temperature').value() + ' K');select('#particles-value').html(select('#particles').value() + ' 个');select('#volume-value').html(select('#volume').value() + ' px');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('气体分子热运动', '热力学温度 K'); drawScene(p); }
