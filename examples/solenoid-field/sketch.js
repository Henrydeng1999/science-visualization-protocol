...
function readParams(){return {current: parseFloat(select('#current').value()),turns: parseFloat(select('#turns').value()),length: parseFloat(select('#length').value())};}
function setupControls(){select('#current').input(updateLabels);select('#turns').input(updateLabels);select('#length').input(updateLabels);}
function updateLabels(){select('#current-value').html(select('#current').value() + ' A');select('#turns-value').html(select('#turns').value() + ' 匝');select('#length-value').html(select('#length').value() + ' px');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('螺线管磁场', '电流 A'); drawScene(p); }
