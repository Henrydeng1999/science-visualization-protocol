...
function readParams(){return {m1: parseFloat(select('#m1').value()),v1: parseFloat(select('#v1').value()),m2: parseFloat(select('#m2').value()),offset: parseFloat(select('#offset').value()),elastic: parseFloat(select('#elastic').value())};}
function setupControls(){select('#m1').input(updateLabels);select('#v1').input(updateLabels);select('#m2').input(updateLabels);select('#offset').input(updateLabels);select('#elastic').input(updateLabels);select('#restart').mousePressed(restartCollision);}
function updateLabels(){select('#m1-value').html(select('#m1').value() + ' kg');select('#v1-value').html(select('#v1').value() + ' m/s');select('#m2-value').html(select('#m2').value() + ' kg');select('#offset-value').html(select('#offset').value() + ' px');select('#elastic-value').html(select('#elastic').value() + '');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); restartCollision(); }
function restartCollision(){ activeParams=readParams(); simStartFrame=frameCount; lastCycleIndex=0; }
function draw(){ const p=activeParams||readParams(); background(12,16,28); drawFrame('碰撞与动量守恒', '质量 kg'); drawScene(p); }
...
function drawCollision(p){
  const r1=28, r2=28;
  const cycle=180, elapsed=max(0,frameCount-simStartFrame), cycleIndex=floor(elapsed/cycle);
  ...
  const localElapsed=max(0,frameCount-simStartFrame), phase=(localElapsed%cycle)/cycle;
  const approachEnd=0.36;
  const bounds={x:56,y:100,w:588,h:300};
  const target=createVector(410,246);
  const maxOffset=r1+r2;
  const offset=constrain(p.offset,-maxOffset,maxOffset);
  const start1=createVector(128,target.y+offset);
  const contact1=getHorizontalContactPoint(target,offset,r1,r2);
  const n=p5.Vector.sub(target,contact1).normalize();
  const tangent=createVector(-n.y,n.x);
  const u1=createVector(p.v1,0), u2=createVector(0,0);
  const u1n=u1.dot(n), u1t=u1.dot(tangent);
  const u2n=0, e=p.elastic;
  const v1n=((p.m1-e*p.m2)*u1n+(1+e)*p.m2*u2n)/(p.m1+p.m2);
  const v2n=((p.m2-e*p.m1)*u2n+(1+e)*p.m1*u1n)/(p.m1+p.m2);
  const v1=p5.Vector.add(p5.Vector.mult(n,v1n),p5.Vector.mult(tangent,u1t));
  const v2=p5.Vector.mult(n,v2n);
  const arrowScale=16;
  let pos1, pos2;

  if(phase<approachEnd){
    const t=phase/approachEnd;
    pos1=p5.Vector.lerp(start1,contact1,easeInQuad(t));
    pos2=target.copy();
  }else{
    const t=(phase-approachEnd)/(1-approachEnd);
    const scale=62;
    pos1=p5.Vector.add(contact1,p5.Vector.mult(v1,scale*t));
    pos2=p5.Vector.add(target,p5.Vector.mult(v2,scale*t));
  }

  drawCollisionGuide(bounds,target,start1,contact1,offset);
  noStroke(); fill(150,165,190); textSize(12); text('二维碰撞示意：左球水平飞来，偏心距决定撞击方向',80,92);
  drawingContext.setLineDash([6,6]);
  stroke(255,235,120,100); strokeWeight(1.5); line(contact1.x,contact1.y,target.x,target.y);
  drawingContext.setLineDash([]);
  noStroke(); fill(255,200,90); circle(pos1.x,pos1.y,r1*2);
  fill(120,200,255); circle(pos2.x,pos2.y,r2*2);

  if(phase>=approachEnd&&phase<approachEnd+0.04){
    stroke(255,235,120); strokeWeight(3); noFill();
    for(let a=-0.8;a<=0.8;a+=0.4) line(target.x,target.y,target.x+28*cos(a),target.y+28*sin(a));
  }

  if(phase<approachEnd) drawVectorArrow(pos1.x,pos1.y,u1.copy().mult(arrowScale),color(255,200,90),'碰前 v₁');
  if(phase>=approachEnd){
    drawVectorArrow(pos1.x,pos1.y,v1.copy().mult(arrowScale),color(255,200,90),'v₁′');
    drawVectorArrow(pos2.x,pos2.y,v2.copy().mult(arrowScale),color(120,200,255),'v₂′');
  }
  const pBefore=p.m1*p.v1;
  const pAfterX=p.m1*v1.x+p.m2*v2.x;
  const pAfterY=p.m1*v1.y+p.m2*v2.y;
  caption(['拖动参数后需点击“重新开始”才会应用到本次碰撞。','总动量：碰前 px='+pBefore.toFixed(1)+'，碰后 px='+pAfterX.toFixed(1)+'，py='+pAfterY.toFixed(1)+'。']);
}
function easeInOut(t){ return t*t*(3-2*t); }
function easeOutCubic(t){ return 1-pow(1-t,3); }
function easeInQuad(t){ return t*t; }
function getHorizontalContactPoint(target,offset,r1,r2){
  const contactDistance=r1+r2;
  const dx=sqrt(max(0,contactDistance*contactDistance-offset*offset));
  return createVector(target.x-dx,target.y+offset);
}
function drawCollisionGuide(bounds,target,start1,contact1,offset){
  stroke(45,56,78);
  strokeWeight(1);
  line(bounds.x,bounds.y+bounds.h,bounds.x+bounds.w,bounds.y+bounds.h);
  drawingContext.setLineDash([7,7]);
  stroke(120,170,230,90);
  line(start1.x,start1.y,contact1.x,contact1.y);
  stroke(255,235,120,90);
  line(target.x,target.y,target.x,target.y+offset);
  drawingContext.setLineDash([]);
  noStroke();
  fill(120,170,230);
  textSize(11);
  text('入射轨迹',start1.x+10,start1.y-10);
  fill(255,235,120);
  text('偏心距',target.x+10,target.y+offset/2);
}
function drawVectorArrow(x,y,v,c,label){
  if(v.mag()<2) return;
  const x2=x+v.x, y2=y+v.y;
  stroke(c); strokeWeight(3); line(x,y,x2,y2);
  const a=atan2(v.y,v.x);
  noStroke(); fill(c);
  push(); translate(x2,y2); rotate(a); triangle(0,0,-9,-5,-9,5); pop();
  fill(210); textSize(11); text(label,x2+6,y2-6);
}
