...
function readParams(){return {type: select('#circuit-type').value(),voltage: parseFloat(select('#voltage').value()),resistance: parseFloat(select('#resistance').value()),capacitance: parseFloat(select('#capacitance').value()),inductance: parseFloat(select('#inductance').value()),closed: select('#switch').elt.checked};}
function setupControls(){select('#circuit-type').changed(updateLabels);select('#voltage').input(updateLabels);select('#resistance').input(updateLabels);select('#capacitance').input(updateLabels);select('#inductance').input(updateLabels);select('#switch').changed(updateLabels);}
function updateLabels(){const names={resistor:'简单电阻电路',capacitor:'RC 电容充电',inductor:'RL 电感电路'};select('#type-value').html(names[select('#circuit-type').value()]);select('#voltage-value').html(parseFloat(select('#voltage').value()).toFixed(1) + ' V');select('#resistance-value').html(parseFloat(select('#resistance').value()).toFixed(1) + ' Ω');select('#capacitance-value').html(parseFloat(select('#capacitance').value()).toFixed(1) + ' F');select('#inductance-value').html(parseFloat(select('#inductance').value()).toFixed(1) + ' H');select('#switch-value').html(select('#switch').elt.checked?'闭合：电路接通':'断开：没有持续电流');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('简单电路', '电流 A'); drawScene(p); }
...
function drawCircuit(p){
  const time=(frameCount%360)/60;
  const tauRC=max(0.2,p.resistance*p.capacitance/8);
  const tauRL=max(0.2,p.inductance/p.resistance*4);
  const steady=p.voltage/p.resistance;
  let current=p.closed?steady:0;
  let power=current*p.voltage;
  let brightness=constrain(map(power,0,80,30,255),30,255);
  let stored=0;
  if(p.type==='capacitor'){
    const charge=p.closed?1-exp(-time/tauRC):0;
    stored=0.5*p.capacitance*p.voltage*p.voltage*charge*charge;
    current=p.closed?steady*exp(-time/tauRC):0;
    power=current*p.voltage;
    brightness=constrain(map(current,0,steady,40,220),40,220);
  }else if(p.type==='inductor'){
    const build=p.closed?1-exp(-time/tauRL):0;
    current=steady*build;
    power=current*p.voltage;
    stored=0.5*p.inductance*current*current;
    brightness=constrain(map(current,0,steady,40,240),40,240);
  }
  const left=120,right=585,top=145,bottom=365;
  drawCircuitWires(left,right,top,bottom,p.closed,current);
  drawBattery(left,255,p.voltage);
  drawSwitch(352,top,p.closed);
  if(p.type==='resistor') drawLamp(right,255,brightness,current);
  if(p.type==='capacitor') drawCapacitor(right,255,p.capacitance,p.closed?1-exp(-time/tauRC):0);
  if(p.type==='inductor') drawInductor(right,255,p.inductance,current);
  drawResistor(300,bottom,p.resistance);
  drawCurrentDots(left,right,top,bottom,current,p.closed);
  drawMeters(p,current,power,stored,time);
  if(p.type==='capacitor') caption(['RC 电路：闭合瞬间电流较大，随后电容电压逐渐接近电源电压，电流逐渐变小。','时间常数 τ = RC，电阻或电容越大，充电越慢。']);
  else if(p.type==='inductor') caption(['RL 电路：电感会阻碍电流突变，闭合后电流从 0 逐渐增大到 U/R。','时间常数 τ = L/R，电感越大建立越慢，电阻越大建立越快但稳态电流更小。']);
  else caption(['欧姆定律：I = U/R = '+current.toFixed(2)+' A。闭合开关后才形成持续电流。','电压越大，电流和灯泡亮度越强；电阻越大，电流越小，灯泡越暗。']);
}

function drawCircuitWires(left,right,top,bottom,closed,current){
  stroke(closed?color(120,220,255):color(72,86,112));
  strokeWeight(5);
  noFill();
  line(left,255,left,top);
  line(left,top,320,top);
  line(384,top,right,top);
  line(right,top,right,218);
  line(right,292,right,bottom);
  line(right,bottom,354,bottom);
  line(246,bottom,left,bottom);
  line(left,bottom,left,255);
  if(closed){
    drawCircuitArrow(198,top,1,0,color(255,210,100));
    drawCircuitArrow(right,180,0,1,color(255,210,100));
    drawCircuitArrow(470,bottom,-1,0,color(255,210,100));
    drawCircuitArrow(left,314,0,-1,color(255,210,100));
  }
}
...
function drawBattery(x,y,voltage){
  stroke(230);
  strokeWeight(3);
  line(x-16,y-54,x-16,y+54);
  line(x+16,y-34,x+16,y+34);
  stroke(120,220,255);
  strokeWeight(5);
  line(x,y-92,x,y-58);
  line(x,y+58,x,y+92);
  ...
  fill(220);
  textSize(12);
  text('电池 '+voltage.toFixed(1)+' V',x-42,y+84);
}
...
function drawSwitch(x,y,closed){
  ...
  stroke(255,210,100);
  strokeWeight(4);
  if(closed) line(x-8,y,x+30,y);
  else line(x-8,y,x+26,y-32);
  noStroke();
  fill(closed?color(120,240,170):color(255,150,120));
  textSize(12);
  text(closed?'开关闭合':'开关断开',x-36,y-44);
}

function drawLamp(x,y,brightness,current){
  noStroke();
  fill(255,220,80,brightness*0.18);
  circle(x,y,118);
  fill(255,220,80,brightness*0.35);
  circle(x,y,82);
  stroke(230);
  strokeWeight(3);
  fill(20,26,36);
...
function drawCapacitor(x,y,capacitance,charge){
  stroke(230);
  strokeWeight(4);
  line(x-22,y-42,x-22,y+42);
  line(x+22,y-42,x+22,y+42);
  stroke(120,220,255);
  strokeWeight(5);
  line(x,y-92,x,y-48);
  line(x,y+48,x,y+92);
  noStroke();
  fill(120,220,255,80+charge*150);
  rect(x-52,y-44,22,88,5);
  fill(255,210,100,80+charge*150);
  rect(x+30,y-44,22,88,5);
  fill(255,210,100);
  textSize(16);
  text('+',x+58,y-26);
  fill(120,220,255);
  ...
  fill(220);
  textSize(12);
  text('电容 C = '+capacitance.toFixed(1)+' F',x-50,y+78);
  fill(150,165,190);
  text('充电 '+floor(charge*100)+'%',x-34,y+96);
}

function drawInductor(x,y,inductance,current){
  stroke(120,220,255);
  strokeWeight(5);
  line(x,y-92,x,y-52);
  line(x,y+52,x,y+92);
  ...
  noFill();
  for(let i=0;i<5;i++){
    arc(x,y-34+i*17,42,30,-HALF_PI,HALF_PI);
  }
  const glow=constrain(current*38,0,120);
  noStroke();
  fill(120,200,255,glow);
  ellipse(x,y,92,132);
  fill(220);
  textSize(12);
  text('电感 L = '+inductance.toFixed(1)+' H',x-50,y+78);
  fill(150,165,190);
  text('磁场储能',x-28,y+96);
}
...
function drawResistor(x,y,resistance){
  ...
  noFill();
  beginShape();
  const start=x-54, step=18;
  vertex(start,y);
  for(let i=0;i<7;i++){
    vertex(start+i*step+step/2,y+(i%2===0?-16:16));
  }
  vertex(x+78,y);
  endShape();
  noStroke();
  fill(220);
  textSize(12);
  text('电阻 R = '+resistance.toFixed(1)+' Ω',x-48,y+38);
}

function drawCurrentDots(left,right,top,bottom,current,closed){
  if(!closed||current<=0) return;
  const speed=1.2+current*0.45;
  const count=constrain(floor(10+current*8),10,34);
  const path=[
    createVector(left,top),createVector(right,top),
    createVector(right,bottom),createVector(left,bottom),
    createVector(left,top)
  ];
  const lengths=[0];
  let total=0;
  for(let i=1;i<path.length;i++){total+=p5.Vector.dist(path[i-1],path[i]);lengths.push(total);}
  for(let i=0;i<count;i++){
    const d=(frameCount*speed+i*total/count)%total;
    const pt=pointOnPolyline(path,lengths,d);
    noStroke();
    fill(120,240,170,190);
    circle(pt.x,pt.y,6);
  }
}

function pointOnPolyline(path,lengths,d){
  for(let i=1;i<path.length;i++){
    if(lengths[i]>=d){
      const t=(d-lengths[i-1])/(lengths[i]-lengths[i-1]);
      return p5.Vector.lerp(path[i-1],path[i],t);
    }
  }
  return path[0].copy();
}

function drawMeters(p,current,power,stored,time){
  noStroke();
  fill(18,25,40,235);
  rect(268,196,164,104,8);
  fill(220);
  textSize(12);
  text('U='+p.voltage.toFixed(1)+' V  R='+p.resistance.toFixed(1)+'Ω',280,218);
  fill(120,220,255);
  text('I='+current.toFixed(2)+' A',280,240);
  fill(255,210,100);
  text('P='+power.toFixed(1)+' W',280,262);
  fill(150,165,190);
  text(p.type==='resistor'?'稳态电路':('储能≈'+stored.toFixed(1)),280,284);
}

function drawCircuitArrow(x,y,dx,dy,c){
  stroke(c);
  strokeWeight(2);
