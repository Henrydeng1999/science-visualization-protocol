...
function readParams(){return {mass: parseFloat(select('#mass').value()),speed: parseFloat(select('#speed').value())};}
function setupControls(){select('#mass').input(updateLabels);select('#speed').input(updateLabels);}
function updateLabels(){select('#mass-value').html(parseFloat(select('#mass').value()).toFixed(1) + ' 相对值');select('#speed-value').html(parseFloat(select('#speed').value()).toFixed(1) + ' 相对值');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('德布罗意波长', '长度 m · 质量 kg'); drawScene(p); }
...
function drawDebroglie(p){
  const momentum=p.mass*p.speed;
  const range=getMomentumRange();
  const momentumRatio=getMomentumRatio(momentum,range);
  const lambda=15+205*(1-pow(momentumRatio,0.74));
  const x0=78, x1=620, midY=245;
  const visualSpeed=0.65+0.018*p.speed;
  const travel=(frameCount*visualSpeed)%(x1-x0);
  const particleX=x0+travel;
  const packetWidth=58;

  drawDebroglieReadout(p,momentum,lambda);
  drawMomentumBar(momentum,range);

  stroke(70,88,115);
  strokeWeight(2);
  line(x0,midY,x1,midY);
  noStroke();
  fill(145,160,185);
  textSize(12);
  text('跟随粒子的物质波包',x0,midY-88);

  noFill();
  stroke(120,220,255);
  strokeWeight(3);
  drawWavePacket(x0,x1,midY,particleX,lambda,packetWidth);

  stroke(255,210,100);
  strokeWeight(2);
  const markX=x0+28;
  line(markX,midY+92,markX+lambda,midY+92);
  drawSmallTick(markX,midY+82,midY+102,color(255,210,100));
  drawSmallTick(markX+lambda,midY+82,midY+102,color(255,210,100));
  noStroke();
  fill(255,210,100);
  textSize(13);
  text('λ ≈ '+lambda.toFixed(1)+' px',markX+lambda/2-34,midY+118);

  fill(255,210,100);
  circle(particleX,midY,18);
  fill(255,240,170);
  circle(particleX-5,midY-5,6);
  const vArrow=constrain(18+p.speed*0.8,22,70);
  drawArrow(particleX+18,midY,particleX+18+vArrow,midY,color(255,210,100));
  noStroke();
  fill(255,210,100);
  textSize(12);
  text('粒子运动方向 v',particleX+24+vArrow,midY-8);

  drawFormulaCard(p,momentum,lambda);
  caption(['德布罗意关系：λ = h/p，本页用相对尺度演示。','质量或速度越大，动量 p = mv 越大，波长 λ 越短，波峰会变得更密。']);
}

function drawWavePacket(x0,x1,midY,centerX,lambda,packetWidth){
  const left=max(x0,centerX-packetWidth*2.25);
  const right=min(x1,centerX+packetWidth*2.25);
  drawGaussianEnvelope(left,right,midY,centerX,packetWidth);

  noFill();
  stroke(120,220,255);
  strokeWeight(3);
  beginShape();
  const sampleStep=constrain(lambda/72,0.045,0.28);
  for(let x=left;x<=right;x+=sampleStep){
    const envelope=exp(-sq(x-centerX)/(2*packetWidth*packetWidth));
    const carrier=sin(TWO_PI*(x-centerX)/lambda);
    vertex(x,midY+62*envelope*carrier);
  }
  endShape();
}

function drawDebroglieReadout(p,momentum,lambda){
  noStroke();
  fill(18,25,40,235);
  rect(462,86,178,98,8);
  fill(220);
  textSize(13);
  text('m = '+p.mass.toFixed(1),478,112);
  text('v = '+p.speed.toFixed(1),478,134);
  text('p = mv = '+momentum.toFixed(1),478,156);
  fill(120,220,255);
  text('λ 随 p 增大而缩短：'+lambda.toFixed(1)+' px',478,176);
}

function getMomentumRange(){
  const mMin=parseFloat(select('#mass').elt.min);
  const mMax=parseFloat(select('#mass').elt.max);
  const vMin=parseFloat(select('#speed').elt.min);
  const vMax=parseFloat(select('#speed').elt.max);
  return {min:mMin*vMin,max:mMax*vMax};
}

function getMomentumRatio(momentum,range){
  const safeMin=max(0.001,range.min);
  const safeMax=max(safeMin*1.01,range.max);
  return constrain((log(momentum)-log(safeMin))/(log(safeMax)-log(safeMin)),0,1);
}

function drawMomentumBar(momentum,range){
  const x=462,y=204,w=178,h=50;
  const ratio=getMomentumRatio(momentum,range);
  ...
  fill(255,160,120);
  rect(barX,barY,ratio*barW,8,4);
  fill(255,210,100);
  circle(barX+ratio*barW,barY+4,8);
  fill(105,120,145);
  textSize(10);
  text('min '+range.min.toFixed(0),x+14,y+47);
  text('max '+range.max.toFixed(0),x+w-58,y+47);
}

function drawGaussianEnvelope(left,right,midY,centerX,packetWidth){
  noStroke();
  fill(120,220,255,28);
  beginShape();
  for(let x=left;x<=right;x+=1){
    const envelope=exp(-sq(x-centerX)/(2*packetWidth*packetWidth));
    vertex(x,midY-64*envelope);
  }
...
function drawFormulaCard(p,momentum,lambda){
  const x=432,y=314,w=208,h=84;
  noStroke();
  fill(18,25,40,235);
  rect(x,y,w,h,8);
  fill(220);
  ...
  text('关系链',x+14,y+24);
  fill(255,210,100);
  text('m × v = p',x+14,y+46);
  fill(120,220,255);
  text('λ = h / p',x+100,y+46);
  fill(150,165,190);
  textSize(12);
  text('现在：p='+momentum.toFixed(0)+'，λ≈'+lambda.toFixed(1)+' px',x+14,y+68);
}

function drawSmallTick(x,y1,y2,c){
  stroke(c);
  strokeWeight(2);
  line(x,y1,x,y2);
}
