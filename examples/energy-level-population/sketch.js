...
function readParams(){return {temperature: parseFloat(select('#temperature').value()),gap: parseFloat(select('#gap').value()),particles: parseFloat(select('#particles').value())};}
function setupControls(){select('#temperature').input(updateLabels);select('#gap').input(updateLabels);select('#particles').input(updateLabels);}
function updateLabels(){select('#temperature-value').html(select('#temperature').value() + ' K');select('#gap-value').html(select('#gap').value() + ' 相对值');select('#particles-value').html(select('#particles').value() + '');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('能级布居', '物质的量 mol · 热力学温度 K'); drawScene(p); }
