...
function readParams(){return {angle: parseFloat(select('#angle').value()),intensity: parseFloat(select('#intensity').value())};}
function setupControls(){select('#angle').input(updateLabels);select('#intensity').input(updateLabels);}
function updateLabels(){select('#angle-value').html(select('#angle').value() + ' °');select('#intensity-value').html(select('#intensity').value() + '');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('偏振', '发光强度 cd'); drawScene(p); }
