...
function readParams(){return {wavelength: parseFloat(select('#wavelength').value()),slit: parseFloat(select('#slit').value()),distance: parseFloat(select('#distance').value())};}
function setupControls(){select('#wavelength').input(updateLabels);select('#slit').input(updateLabels);select('#distance').input(updateLabels);}
function updateLabels(){select('#wavelength-value').html(select('#wavelength').value() + ' px');select('#slit-value').html(select('#slit').value() + ' px');select('#distance-value').html(select('#distance').value() + ' px');}

function setup(){ const canvas=createCanvas(700,500); canvas.parent('canvas-container'); setupControls(); updateLabels(); }
function draw(){ const p=readParams(); background(12,16,28); drawFrame('光的衍射', '长度 m · 发光强度 cd'); drawScene(p); }
...
function drawDiffraction(p){ const slitX=260, screenX=560, cy=250; stroke(170); strokeWeight(6); line(slitX,80,slitX,cy-p.slit/2); line(slitX,cy+p.slit/2,slitX,420); stroke(120,200,255,90); noFill(); for(let r=(frameCount%p.wavelength);r<330;r+=p.wavelength) arc(slitX,cy,r*2,r*2,-HALF_PI,HALF_PI); noStroke(); const gain=3; for(let y=90;y<410;y+=3){ let theta=atan((y-cy)/p.distance), beta=gain*PI*p.slit*sin(theta)/p.wavelength, I=beta===0?1:pow(sin(beta)/beta,2), disp=pow(I,0.35); fill(255*disp,220*disp,80); rect(screenX,y,48,3); } caption(['狭缝越窄或波长越长，衍射越明显。','屏幕上的亮暗来自不同方向的波叠加。']); }
